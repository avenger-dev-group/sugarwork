...
Trusted pull requests and master pushes also run `--scenario sdk-live --installed-wheel` on each selected native target. That scenario performs two tool-using turns through `metis/metis-coder-max`: it checks the created file immediately, replaces its content with a host-only random challenge, and requires the second turn to copy the changed content into a fresh receipt without modifying the source. Both turns must complete after a model-requested tool call; external byte comparisons, rather than provider-generated acknowledgement wording, check the result. Missing repository secrets fail instead of self-skipping. Fork and Dependabot pull requests run the complete keyless installed-wheel path but receive no key.
...
Alternatively export a non-empty `DSH_HOME`. The SDK rejects a launch that would silently use `~/.sw`.
