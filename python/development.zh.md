...
可信拉取请求与 master 推送还会在各自选定的原生目标上运行 `--scenario sdk-live --installed-wheel`。该场景通过 `metis/metis-coder-max` 执行两个使用工具的轮次：立即检查已创建文件，将其内容替换为仅宿主知道的随机挑战值，并要求第二轮将变更后的内容复制到全新的回执文件，且不修改源文件。两个轮次都必须在模型请求工具调用后完成；测试通过外部逐字节比较而非提供方生成的确认文字来验证结果。仓库密钥缺失时失败，而不是自行 skip。Fork 与 Dependabot 拉取请求会运行完整的 keyless 安装后 wheel 路径，但不会获得密钥。
...
也可以导出非空 `DSH_HOME`。SDK 会拒绝可能静默使用 `~/.sw` 的启动。
