...
class HarnessConfig:
    """Configuration for launching the local SugarWork SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the SugarWork SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        if proc.poll() is None:
            try:
                ...
                proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("SugarWork runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for SugarWork runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("SugarWork runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to SugarWork runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("SugarWork runtime stdout closed"))
    ...
    def _default_launch_args(self, env: dict[str, str]) -> tuple[str, ...]:
        if self.config.dsh_bin is None:
            try:
                ...
                raise FileNotFoundError(
                    "Unable to locate the bundled SugarWork dsh runtime. "
        ...
        if self.config.dsh_home is not None:
            ...
            raise ValueError(
                ...
                "the Python SDK never uses ~/.sw implicitly"
            )
