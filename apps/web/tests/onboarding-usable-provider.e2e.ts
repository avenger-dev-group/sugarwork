...
// for the default Metis key again, and the first-run setup card is a card
// they can close. The shipped Metis route stays mounted without a
...
describe.skipIf(MODE === 'record')('web e2e: another usable provider ends first-run onboarding', () => {
  ...
  it('dismisses setup while keeping Metis available beside an add-provider draft', async () => {
    ...
    await settings.getByRole('button', { name: '编辑 Metis (metis)' }).waitFor({ timeout: 10_000 })
  ...
  it('stops prompting for Metis once the other provider can serve requests', async () => {
    ...
    // Only minimax-cn is reachable; Metis still holds no credential.
    ...
    expect(credentials).not.toContain('METIS_API_KEY')
    ...
    // The Models page agrees: Metis stays a row rather than reopening its
