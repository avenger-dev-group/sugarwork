...
describe('web e2e: workspace welcome screen', () => {
  ...
  it('enters without credentials and retains entry after reload', async () => {
    ...
    await page.getByRole('heading', { name: 'Good morning, SIMON.F' }).waitFor()
    expect(await page.getByRole('button', { name: 'Dashboard' }).count()).toBe(1)
    expect(await page.getByRole('button', { name: 'Existing Customers' }).count()).toBe(1)
    expect(await page.getByText('Attendance today').count()).toBe(1)
    await page.getByRole('button', { name: 'Existing Customers' }).click()
    await page.getByRole('heading', { name: 'Existing Customers' }).waitFor()
    expect(await page.getByRole('button', { name: 'Back to Dashboard' }).count()).toBe(0)
    await page.getByRole('searchbox', { name: 'Search customer master data' }).fill('Westlake')
    expect(await page.getByText('Westlake Hospital').count()).toBe(1)
    expect(await page.getByText('BrightCare Medical').count()).toBe(0)
    await page.getByRole('button', { name: 'Add customer' }).click()
    await page.getByRole('dialog', { name: 'Add customer' }).waitFor()
    await page.getByRole('button', { name: 'Close customer editor' }).click()
    await page.getByRole('button', { name: 'Call Logs' }).click()
    await page.getByRole('heading', { name: 'Call Logs' }).waitFor()
    await page.getByRole('button', { name: 'Details BrightCare Medical' }).click()
    await page.getByRole('complementary', { name: 'Call details' }).waitFor()
    expect(await page.getByText('AI call summary').count()).toBe(1)
