...
| State ownership | Sharing executable dependency graphs would let CLI and Desktop change each other's dsh, Cordis, plugin, or native-module versions, while two desktop processes could race on the same profile. | Electron acquires its process-lifetime single-instance lock before any profile access and exclusively owns `$SW_HOME/profiles/desktop` plus its package-manager state. CLI and Desktop share supported product data under `$SW_HOME`, but never executable packages, plugin activation, lockfiles, or `node_modules`. |
...
Electron chooses typed English or Chinese shell copy from its application locale and falls back to English. Menus, native dialogs, the startup page, and the plugin-management renderer use the same locale payload; the repository Client UI i18n gate checks these desktop sources. The application menu includes Electron's native Edit roles so focused text and secret inputs retain undo, cut, copy, paste, and select-all shortcuts on every supported platform.
...
`DSH_DESKTOP_AUTO_UPDATE_ENV` selects `test` or `production` for both the URL embedded during packaging and the later COS upload; an absent value selects `test`. Packaging requires the selected HTTPS origin in `DOWNLOAD_TEST_ORIGIN` or `DOWNLOAD_PROD_ORIGIN`, so neither deployment has a hard-coded update service. Upload additionally requires the selected deployment's COS bucket in `DOWNLOAD_TEST_COS_BUCKET` or `DOWNLOAD_PROD_COS_BUCKET`. The target path is `_/sugarwork/desktop/stable/<target>/`, where `target` is `mac-arm64`, `mac-x64`, or `win-x64`.
...
- The desktop shell shares sessions, settings, credentials, workspaces, and storage under `$SW_HOME` with CLI dsh, while executable packages, plugin activation, lockfiles, and package-manager state remain separate.
