...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('sw --profile web')
    expect(help.stdout).toContain('sw plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    ...
    try {
      ...
      expect(web.stdout).toContain('Usage: sw --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('sw web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('sw web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: sw --profile headless')
      ...
      expect(sdkHelp.stdout).toContain('Usage: sw --profile sdk')
      ...
      expect(acpHelp.stdout).toContain('Usage: sw --profile acp')
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      ...
      expect(result.stderr).toContain('sw plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS + 30_000)
  ...
  it('creates a custom profile from the shipped web template before booting it', async () => {
    ...
    try {
      ...
      expect(created.stdout).toContain('Usage: sw --profile web')
      ...
      expect(reopened.stdout).toContain('Usage: sw --profile web')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, SPAWN_TIMEOUT_MS * 3 + 30_000)
  ...
  it('keeps a newly created profile when application boot rejects its arguments', async () => {
    ...
    try {
      ...
      expect(retried.stdout).toContain('Usage: sw --profile web')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `sw plugin --profile x add .` from a plugin checkout must install THAT
