...
/**
 * Keyless smoke for SOURCE `sw` execution: run `apps/cli/src/bin.ts`
 * with the exact production runtime vector (`node --import tsx/esm`, the
 * vector the root `sw` script invokes directly) and assert the
 ...
 * breaks this gate instead of every developer's `pnpm sw`; the built-bin
...
describe('sw SOURCE launcher (node --import tsx/esm)', () => {
  it('launches the source CLI without building', async () => {
    const rootPackage = JSON.parse(await readFile(new URL('../../../package.json', import.meta.url), 'utf8')) as {
      readonly scripts?: Record<string, string>
    }
    expect(rootPackage.scripts?.sw).toBe('node --import tsx/esm apps/cli/src/bin.ts')
    expect(rootPackage.scripts?.dsh).toBe(rootPackage.scripts?.sw)
  })
  ...
  it('boots the source entry and requires a profile', async () => {
    ...
    if (result.timedOut) {
      throw new Error(`sw source launch did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
