...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"sw web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed sw web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed sw web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@deepseek-ai/') || name === '@sugarwork-ai/sw-root')) {
