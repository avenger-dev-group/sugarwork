---
description: "SugarWork 主目录与用户数据路径的共享解析，供需要统一根目录、波浪号展开与稳定监听路径的包使用。"
...
`@deepseek-ai/dsh-home-paths` 让包作者能够解析统一的 SugarWork 数据根目录，并由它派生子路径。显式路径优先于 `$SW_HOME`，随后是兼容别名 `$DSH_HOME` 和 `~/.sw`；空白环境变量会被忽略。其公开辅助函数可以在不暴露机器绝对路径的情况下显示根目录，仅展开单独或当前用户的波浪号形式，并规范化最终路径段尚不存在的监听目标。请把它作为库依赖直接使用，不要通过 `cordis.yml` 加载。
...
显式配置的路径优先级最高，然后是 `$SW_HOME`、兼容别名 `$DSH_HOME`，最后是默认的 `~/.sw`。空或仅含空白的值视为未设置，因此空白的覆盖值绝不会把主目录解析到当前工作目录。
...
面向用户的路径请以符号形式渲染根目录，而不是机器路径：默认主目录显示为 `~/.sw`，任何已配置的主目录显示为 `$SW_HOME`。展示形式绝不会泄露机器的绝对路径。
...
`resolveDshHome` 依次读取显式覆盖值、`$SW_HOME` 和兼容别名 `$DSH_HOME`，最后回退到操作系统主目录拼接 `.sw`。选中的值经过波浪号展开并规范化为绝对路径；`dshHomePath` 用 Node 的平台路径规则拼接子路径段。`dshHomeDisplay` 把解析出的路径与默认根目录比较并返回 SugarWork 的规范标签，因此已配置的主目录绝不泄露其绝对路径。
...
- [shell 环境](../../shell/shell-env/README.zh.md)——`SW_HOME` 如何到达模型 shell 调用。
