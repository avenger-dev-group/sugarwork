---
description: "Shared resolution of the SugarWork home and user-data paths for packages that need one consistent root, tilde expansion, and stable watch paths."
...
`@deepseek-ai/dsh-home-paths` lets package authors resolve one SugarWork data root and derive child paths from it. An explicit path wins over `$SW_HOME`, followed by the compatibility `$DSH_HOME` alias and then `~/.sw`; blank environment values are ignored. Its public helpers can render the root without revealing an absolute machine path, expand only bare or current-user tilde forms, and canonicalize watch targets whose final components do not yet exist. Use it as a direct library dependency, not through `cordis.yml`.
...
const home = resolveDshHome()                // configured path, else $SW_HOME, else ~/.sw
const settings = dshHomePath('settings')     // join one child onto the resolved home
const cache = dshCachePath('models')         // $SW_HOME/cache/models, default ~/.sw/cache/models
```

An explicit configured path has the highest precedence, then `$SW_HOME`, the compatibility `$DSH_HOME` alias, and the default `~/.sw`. Empty or whitespace-only values are treated as unset, so a blank override never resolves the home to the current working directory.
...
For user-facing paths, render the root symbolically rather than as a machine path: the default home displays as `~/.sw` and any configured home displays as `$SW_HOME`. The display form never leaks an absolute machine path.
...
`resolveDshHome` reads the explicit override, `$SW_HOME`, and the compatibility `$DSH_HOME` alias before falling back to the operating-system home joined with `.sw`. The chosen value is tilde-expanded and normalized to an absolute path; `dshHomePath` joins child segments with Node's platform path rules. `dshHomeDisplay` compares the resolved path against the default root and returns the canonical SugarWork label, so a configured home never leaks its absolute path.
...
- [Shell environment](../../shell/shell-env/README.md) — how `SW_HOME` reaches model shell calls.
