...
describe('workspace context instruction discovery', () => {
  ...
  it('defaults dshHome and uses cwd itself as root when no project marker exists', async () => {
    ...
    // the home dirs point at an empty dir so the default ~/.sw holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.sw/AGENTS.md would otherwise leak in.
    vi.stubEnv('DSH_HOME', '')
  ...
  it('honors DSH_HOME when dshHome is not configured explicitly', async () => {
    ...
    try {
      ...
      expect(files).toEqual([{ absolutePath: join(envHome, 'AGENTS.md'), displayPath: '$SW_HOME/AGENTS.md' }])
    } finally {
      vi.unstubAllEnvs()
      await rm(root, { recursive: true, force: true })
  ...
  it('labels the default SugarWork home as ~/.sw when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.sw/AGENTS.md'), 'global default rule')

      // A set SW_HOME would override the homedir default and relabel the home.
      vi.stubEnv('SW_HOME', '')
      vi.resetModules()
      ...
      expect(files.map(file => file.displayPath)).toEqual(['~/.sw/AGENTS.md'])
    } finally {
      vi.unstubAllEnvs()
      vi.doUnmock('node:os')
      vi.resetModules()
  ...
  it('expands a configured ~/.sw home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.sw/AGENTS.md'), 'global tilde rule')

      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@deepseek-ai/dsh-agent-instructions')
      const files = await isolated.discoverBaselineInstructionFiles({ cwd: root, dshHome: '~/.sw' })

      expect(files).toEqual([{ absolutePath: join(home, '.sw/AGENTS.md'), displayPath: '~/.sw/AGENTS.md' }])
    } finally {
      vi.doUnmock('node:os')
      vi.resetModules()
      await rm(root, { recursive: true, force: true })
  ...
  it('deduplicates user-global instructions when dshHome points at the project root', async () => {
    ...
    try {
      ...
      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$SW_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('labels a custom dshHome as DSH_HOME instead of pretending it is ~/.sw', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['$SW_HOME/AGENTS.md'])
    } finally {
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
