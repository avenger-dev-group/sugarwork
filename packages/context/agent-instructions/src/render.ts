...
/**
 * File name of the single user-global instruction file under `$SW_HOME`.
 * Discovery (`$SW_HOME/<name>`) and reconciliation (the user-global scope key's
...
export function scopeForDisplayPath(displayPath: string): string {
  if (displayPath === '~/.sw/AGENTS.md' || displayPath === '$SW_HOME/AGENTS.md') return USER_GLOBAL_DIRECTORY
