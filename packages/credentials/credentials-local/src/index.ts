/**
 * File-backed credentials provider over `$SW_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $SW_HOME/.credentials.yaml      (provider-managed, writable)
 * > <invocation cwd>/.env            (read-only fallback)
 * > $SW_HOME/.env                   (read-only fallback)
 * ```
 *
...
/** File-backed credentials provider (`$SW_HOME/.credentials.yaml`). */
