...
const MARKDOWN_FIXTURE = [
  ...
  '| streaming | stable |',
  '',
  '[SugarWork](https://example.com/sugarwork)',
  '',
  '```ts',
  'const markdown = true',
  '```',
...
const WEB_SEARCH_META = {
  answer: 'SugarWork is a plugin-based agent harness on vendored Cordis where **every capability is a plugin**.',
  sources: [
    {
      url: 'https://example.com/sugarwork',
      title: 'SugarWork — plugin-based agent harness',
      snippet: 'Everything is a plugin: session, tools, agent-loop, and LLM adapters all mount on the same Cordis context.',
      publishedAt: '2026-07-01',
    },
    {
      url: 'https://example.com/sugarwork/architecture',
      snippet: 'The capability-seam pattern splits each capability into interface, implementation, and consumer packages.',
    },
    {
      url: 'https://example.com/sugarwork/plugins',
...
const FIXTURE_SYSTEM_PROMPT = '你是 SugarWork 的 fixture 助手。用简洁的中文回答，并在需要时调用工具。'
...
function buildAlphaLog(): SessionEvent[] {
  ...
  toolTurn(
    70,
    'web_search',
    '{"queries":["sugarwork architecture"]}',
    'Search results for SugarWork architecture.',
    WEB_SEARCH_META,
  )
  toolTurn(
    71,
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const directoryTree = new Map<string, string[]>([
    ...
    [`${FIXTURE_HOME}/Documents`, [
      'project', 'sugarwork-iOS', 'sugarwork-android', 'sugarwork-platform',
      'sugarwork-web', 'sugarwork-harness', 'sugarwork-app', 'sugarwork-landing-blog',
    ]],
  ])
