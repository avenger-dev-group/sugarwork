...
本包提供 Web GUI 的欢迎页、三栏 AppFrame、左右栏宽度与 `ctx.layout` 呈现控制。点击“进入工作台”即可挂载应用；账号密码字段仅作展示。右栏先让步以保护中栏空间，全屏由占用方呈现，框架保留宽屏底层轨道。主题呈现器负责配色、别名 token、正文字号与 document 元数据。进入状态只保留在当前浏览器标签页，布局状态则在刷新后重置。
...
root 首先呈现本地化的欢迎页，展示应用内置的 Logo，提供账号密码输入框和“进入工作台”按钮。输入可为空或任意值，字段内容不会被读取或存储。点击按钮后挂载应用，并在当前标签页的 `sessionStorage` 中保留不含身份的进入标记。这只是导航占位，不是身份验证或授权：它没有服务端身份，在用于保护数据前必须替换。
...
- **欢迎页不是安全控制**——它只延后浏览器应用的挂载；正式部署需要服务端验证的身份会话。
