...
本包提供 Web GUI 的浏览器会话模拟登录、三栏 AppFrame、左右栏宽度与 `ctx.layout` 呈现控制。用户名 `simon` 通过前不会挂载应用框架；在正式身份服务替换模拟逻辑前，密码刻意不受限制。右栏先让步以保护中栏空间，全屏由占用方呈现，框架保留宽屏底层轨道。主题呈现器负责配色、别名 token、正文字号与 document 元数据。登录状态只保留在当前浏览器标签页，布局状态则在刷新后重置。
...
root 首先呈现本地化的模拟登录。去除首尾空白后只接受精确的用户名 `simon`，任何密码（包括空值）都会通过，成功登录会在当前标签页的 `sessionStorage` 中保留。这只是导航占位，不是身份验证或授权：它没有服务端身份，在用于保护数据前必须替换。
...
- **模拟登录不是安全控制**——它只延后浏览器应用的挂载，并接受任何密码；正式部署需要服务端验证的身份会话。
