...
describe('ui-settings-models apply', () => {
  ...
  it('registers the models nav entry for declarations before or after apply', async () => {
    ...
    const metis = onboarding.find(entry => entry.options.id === 'metis')!
    expect(metis.component).toBe(DefaultProviderOnboardingDialog)
    expect(metis.options).toMatchObject({ id: 'metis', order: 0 })
    const metisInjected = (
      metis.inject as unknown as () => import('../src/client/DefaultProviderOnboardingDialog.tsx').DefaultProviderOnboardingInjected
    )()
    expect(metisInjected.hooks.models).toBe(injected.controller.store)
    expect(typeof metisInjected.operations.storeCredential).toBe('function')
...
describe('pushed invalidations', () => {
  ...
  it('routes pushed credential invalidation into the shared onboarding join', async () => {
    ...
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const entry = b.slots.entries('settings.onboarding')
      .find(candidate => candidate.options.id === 'metis')!
    const injected = (
      entry.inject as unknown as
      () => import('../src/client/DefaultProviderOnboardingDialog.tsx').DefaultProviderOnboardingInjected
    )()
    injected.controller.store.update((state) => { state.status = 'ready' })
    const load = vi.spyOn(injected.controller, 'load').mockResolvedValue()
    b.remote.emit('credentials/reference-updated', ['METIS_API_KEY'])
