/**
 * Default-provider first-run step. Readiness comes from the same
 ...
 * shipped Metis route. The step reuses that page's credential editor in
...
import styles from './DefaultProviderOnboardingDialog.module.css'

/** Registration-side dependencies of {@link DefaultProviderOnboardingDialog}. */
export interface DefaultProviderOnboardingInjected {
  hooks: {
...
export type DefaultProviderOnboardingDialogProps =
  PropsRuntime<'settings.onboarding'> & InjectFace<DefaultProviderOnboardingInjected>

/* v8 ignore next 3 -- closed-union defaults only defend future source widening */
function assertNever(_value: never): never {
  throw new Error('unexpected default-provider onboarding state')
}

/**
 * Prompt a first-run user for the shipped Metis credential while no
 * provider can serve requests and that credential is writable.
 * @param props - settings-shell owner state and Models feature dependencies.
 * @returns the onboarding modal or null when onboarding needs no intervention.
 */
export function DefaultProviderOnboardingDialog(props: DefaultProviderOnboardingDialogProps): ReactNode {
