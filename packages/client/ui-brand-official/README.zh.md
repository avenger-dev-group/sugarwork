...
自有身份的部署不组合本包，而是组合另一个占据两个侧栏 slot 与首屏 slot 的包。占据 slot 是唯一的组合路径；这里不存在任何品牌配置面。
...
三个填充作为一组声明感知的注册安装：嵌套的 `ctx.slots.inject()` 调用等待侧栏与会话声明，因此无论本行在任一声明者之前还是之后激活，这组注册都能工作；声明消失时所有填充一并撤回，HMR 期间也不会留下残缺的品牌混合。浏览器半部是 [`src/client/index.ts`](src/client/index.ts)；node 半部是一个空 Loader 座位。浏览器标题是构建环境的事（`DSH_CLIENT_TITLE`），不在 slot 系统之内。
