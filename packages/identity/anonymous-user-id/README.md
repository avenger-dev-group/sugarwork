...
The id lives in `$SW_HOME/.anonymous-user-id` (`$SW_HOME` defaults to `~/.sw`) as a plain UUID text file. Delete that file to get a fresh id at the next launch; the running process keeps its current id until it exits. Separate harness homes keep separate ids, and no machine or account detail ever goes into the value.
...
- [dsh-home-paths](../../util/home-paths/README.md) — owns `$SW_HOME` and `~/.sw` resolution.
...
- **No cross-home identity** — different `$SW_HOME` values cannot be correlated.
