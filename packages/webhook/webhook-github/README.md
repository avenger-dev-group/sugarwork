...
The normal Web profile already owns `ctx.webServer`. Mount another `dsh-host-webserver` and this adapter inside a group that isolates only `webServer`; the adapter still inherits credentials and `webhookRuntime`. The [GitHub review example](../../../apps/cli/config/examples/github-review/cordis.yml) uses `127.0.0.1:3081/github` behind a TLS reverse proxy while the UI remains on port 3080.
