...
const IDENTITY = 'You are an AI agent powered by SugarWork.'
...
describe('SystemPrompt', () => {
  ...
  describe('built-in sections', () => {
    ...
    it('registers the harness identity and the configured deployment persona', async () => {
      const ctx = new Context()
      await ctx.plugin(SystemPrompt, { personaPrefix: 'You are SugarWork.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are SugarWork.', 'Be precise.', 'cwd: /tmp', ''])
    expect(assembly.contexts).toEqual([
