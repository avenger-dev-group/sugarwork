...
Use `sw plugin --profile sdk-minimal` to manage persistent external dependencies. Profile, home, and ordered `--patch` files can replace rows or insert bundles above the complete default tree. The shipped template applies patches only at startup.
...
The bundle's single insert is the complete application tree: SDK stdio startup and JSON-RPC serving, one environment-configured DeepSeek adapter, the explicit agent core, local subprocess execution, a platform-selected persistent shell PTY, and uncompressed JSONL persistence under `$SW_HOME/sessions`. It does not inherit another bundle, so every extra row is an explicit profile change.
...
- **The composition intentionally omits shared product services** — select `sw --profile sdk` when settings, managed credentials, policy presets, telemetry, Web tools, or the full default tool roster are required.
