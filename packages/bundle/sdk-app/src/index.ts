...
function sdkCommand(profile: string): Command {
  return new Command()
    .name(`sw --profile ${profile}`)
    .description('Serve SugarWork SDK clients over stdio JSON-RPC.')
    .helpOption('-h, --help', 'show this help')
    .addHelpText('after', `
Example:
  sw --profile ${profile}     serve one SDK runtime until its client disconnects
`)
}
