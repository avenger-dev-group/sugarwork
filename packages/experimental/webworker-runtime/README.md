...
- **`pi-ai` supports catalog reads only**: configured providers and model metadata remain available so the Web profile can activate and render model selectors, while authentication, refresh, and model requests report a named refusal because the worker bundle carries no provider transport.
