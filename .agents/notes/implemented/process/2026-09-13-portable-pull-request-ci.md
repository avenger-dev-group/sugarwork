...
Subprocess-heavy script tests whose executed code is outside the per-file coverage requirement run in the uninstrumented heavy-suite lane. The npm-resolution correctness fixture allows its local npm child 60 seconds to finish without imposing a performance threshold. The DeepSeek streaming fixture sends comments every 400 milliseconds under a one-second idle limit, so the response still proves that comments keep the stream active while retaining scheduling margin under shared CI load. Windows process-tree sampling treats CIM parent relationships as a graph and visits each PID once because PID reuse can form a cycle within one table snapshot.
...
Pull-request jobs can enter GitHub's standard hosted queues without repository-specific runner setup. Resource-proportional concurrency and uninstrumented subprocess fixtures reduce contention on standard machines, while cancellation, fail-fast behavior, child-process deadlines, and job timeouts continue to bound superseded or failed work. Optional self-hosted and Blacksmith routes retain higher parallel throughput when maintainers deliberately select them. Live-provider wording and hosted-runner scheduling variation no longer create false failures, while missing credentials, incomplete turns, absent tool calls, incorrect bytes, source mutation, or an invalid session log still fail the check.
