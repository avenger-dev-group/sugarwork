...
Subprocess-heavy script tests whose executed code is outside the per-file coverage requirement run in the uninstrumented heavy-suite lane. The npm-resolution correctness fixture allows its local npm child 60 seconds to finish without imposing a performance threshold. The DeepSeek streaming fixture sends comments every 400 milliseconds under a one-second idle limit, so the response still proves that comments keep the stream active while retaining scheduling margin under shared CI load.
