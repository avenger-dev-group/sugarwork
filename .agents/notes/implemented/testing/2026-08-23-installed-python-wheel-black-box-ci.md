...
Trusted pull requests and master pushes run a second installed-wheel check on each selected native target with `METIS_API_KEY`, mapped only into a preflight and the live test step. The preflight fails when the secret is empty, so the provider suite cannot self-skip to green. The test starts the public SDK with the shipped `metis/metis-coder-max` route, asks the model to write an exact sentinel file through the platform shell, asks a second turn in the same session to read it, and verifies the external file bytes, completed turn reasons, model-requested tool calls, and the existence and Zstandard framing of its session log. Provider-generated acknowledgement wording is not a correctness observation; deterministic keyless SDK tests own exact final-response projection, while the restart snapshot owns decoded record content and completed-turn durability.
