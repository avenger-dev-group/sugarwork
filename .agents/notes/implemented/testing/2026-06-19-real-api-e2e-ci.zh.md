# Agent Note: 在 CI 中选择启用真实 API e2e 测试
...
一个与 ci.yml 分离的专用工作流 [.github/workflows/e2e.yml](../../../../.github/workflows/e2e.yml) 仅在可信事件上对外部 API 运行 `pnpm run test:e2e`。发行仓库通过 `SUGARWORK_REAL_API_E2E` repository variable 启用它并提供 `SUGARWORK_E2E_API_KEY`；启用后，preflight 检查会将缺失的 secret 转化为明确失败，而非虚假绿色。未配置这些凭证的仓库会干净地跳过该 job。无密钥工作流保持独立，使可 fork 的质量门禁与消费 secret 的真实 API 门禁各自拥有不同的触发和凭证策略。
...
启用后，工作流针对覆盖面和信号优化。它会在多种触发条件和每个受信任 PR（Pull Request）上运行所有匹配的 `*.e2e.ts` 文件，以落实 [docs/testing.md](../../../../docs/testing.zh.md) 的有密钥策略。

### 触发条件：仅限可信事件

`workflow_dispatch` + `push` 到 `main`/`master` + 每夜 `schedule`（`17 0 * * *`，即北京时间 08:17）+ `pull_request`。job 仅在 `SUGARWORK_REAL_API_E2E` 等于 `true` 时运行。push 提供合并后信号；schedule 捕捉外部 API 漂移；dispatch 是手动逃生通道；可信 PR 获得合并前门禁。该合并前信号有意接受 § 安全性中描述的更大密钥暴露面。
...
该门禁是一个*干净跳过的便利措施*，而非 secret 的安全边界（见 § 安全性——边界是 GitHub 自身在 `pull_request` 下对 fork 的 secret 扣留机制）。repository variable 子句还允许没有真实 API 凭证的发行仓库关闭这项可选信号，而不让无关 CI 失败。没有不可信 PR 子句，fork 仍然无法读取密钥；只是会遇到令人困惑的 preflight 硬失败并浪费计算资源。

### Preflight：选择启用后明确失败

repository variable 是明确的启用开关。启用后，preflight 要求 secret 存在：密钥为空→`exit 1` 并附带 `::error::` 注解指明需要配置的 secret 名称。没有它，被删除或错误配置的 secret 会让 `test:e2e` 跳过所有真实套件并报告全绿。尚未启用的仓库会跳过 job，不会把不可用的外部凭证当成产品构建失败。

### Secret 映射与卫生

产品拥有的 repo secret 命名为 `SUGARWORK_E2E_API_KEY`；映射到当前适配器和测试读取的供应商专用 `DEEPSEEK_API_KEY` 环境变量（`process.env.DEEPSEEK_API_KEY`）。仓库级名称标识 CI 使用方，不会把模型供应商呈现为产品身份。以下卫生选择均为防御性设计：
...
- **无写权限（fork PR）：不能。** 两个独立事实阻止了它。第一，工作流使用 `pull_request` 而**非** `pull_request_target`——GitHub 不会将 repo secret 传递给 fork PR 的 `pull_request` 运行，因此 `secrets.SUGARWORK_E2E_API_KEY` 在 fork runner 上解析为空。第二，`if:` 门禁完全跳过 fork PR。secret 扣留是真正的边界；门禁是纵深防御和用户体验。
...
- **翻转时轮换密钥。** 如果密钥曾存在于私有仓库的 CI 中，则将公开视为「假定已暴露」，在那一刻轮换 `SUGARWORK_E2E_API_KEY`。
...
新增一个 CI 工作流和可选的 repo secret。没有外部凭证的发行仓库保持 job 关闭；启用的仓库会获得合并前、合并后、每夜和手动真实 API 覆盖。preflight 会在选择启用后让 secret 配置错误自我通告，而不是静默禁用该信号。
